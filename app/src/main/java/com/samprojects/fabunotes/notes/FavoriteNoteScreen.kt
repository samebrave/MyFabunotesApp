...
@Composable
fun FavoriteNotesScreen(viewModel: NoteViewModel, navController: androidx.navigation.NavController) {
    val favoriteNotes by viewModel.favoriteNotes.collectAsState(initial = emptyList())
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.favorite_notes)) },
                navigationIcon = {
                    IconButton(onClick = {
                        if (selectedNote != null) {
                            selectedNote = null
